...
\item \textbf{Extensionality}. Sets $ A $ and $ B $ are equal iff $ \fab{x}\iffb{\rb{x \in A}}{\rb{x \in B}} $.
\item \textbf{Natural numbers} $ \N = \cb{0, 1, \dots} $. Think of $ 0 = \emptyset $, $ 1 = \cb{0} $, $ 2 = \cb{0, 1} $, etc. Note that for $ m, n \in \N $, $ m < n $ iff $ m \in n $ iff $ m \subset n $.
\item \textbf{Ordered pairs}. The ordered pair $ \rb{x, y} $ is the set $ \cb{\cb{x}, \cb{x, y}} $. For example, for any $ x, y, z, w $, $ \rb{x, y} = \rb{z, w} $ iff $ x = z $ and $ y = w $. If $ A, B $ are sets, then $ A \times B = \cb{\rb{a, b} \mid a \in A, \ b \in B} $. $ A^2 = A \times A $, $ A^3 = A^2 \times A $, etc. The set of \textbf{finite sequences} of elements of $ A $ is $ \cup_{n \in \N} A^n $. $ A^0 = \cb{\emptyset} $.
\item \textbf{Functions}. Think of a function $ f : A \to B $ as a subset of $ A \times B $. $ A = dom\rb{f} $ is the \textbf{domain} and $ B = ran\rb{f} $ is the \textbf{range}. If $ X \subset A $ $ f\sb{X} = \cb{f\rb{a} \mid a \in X} \subseteq B $. Set of functions from $ A $ to $ B $ is $ B^A \subseteq \wp\rb{A \times B} $, where $ \wp $ is the powerset, that is the set of all subsets.
\end{enumerate}
