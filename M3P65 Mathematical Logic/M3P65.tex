...
\item By the Lindenbaum lemma \ref{prop:2.5.2} there is a consistent set $ \Sigma^* \supseteq \Sigma_\infty $ of closed $ \mathcal{L}^+ $-formulas such that for every closed $ \phi $ either $ \Sigma^* \vdash_{K_{\mathcal{L}^+}} \phi $ or $ \Sigma^* \vdash_{K_{\mathcal{L}^+}} \notb{\phi} $.
