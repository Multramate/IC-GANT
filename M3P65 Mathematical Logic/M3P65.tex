...
\item Building a structure. Let $ A = \cb{\overline{t}} $ where $ t $ is a closed term of $ \mathcal{L}^+ $. Note that
\begin{enumerate}
...
\begin{enumerate}
\item Each constant symbol $ c $ of $ \mathcal{L}^+ $ is interpreted as $ \overline{c} \in A $.
\item Suppose $ R $ is an $ n $-ary relation symbol. Define the relation $ \overline{R} \subseteq A^n $ by $ \rb{\overline{t_1}, \dots, \overline{t_n}} \in \overline{R} $ iff $ \Sigma^* \vdash R\rb{t_1, \dots, t_n} $, a closed atomic $ \mathcal{L}^+ $-formula, where $ t_1, \dots, t_n $ are closed $ \mathcal{L}^+ $-terms.
...
\end{enumerate}
Call this structure $ \mathcal{A} $. Note that if $ v $ is a valuation in $ \mathcal{A} $ and $ t $ is a closed term, then $ v\rb{t} = \overline{t} $ by (a) and (c) here.
...
\mathcal{A} \vDash \notb{\psi\rb{b_j}}.
\end{equation}
This contradicts $ \mathcal{A} \vDash \fab{x_i}\psi $. Take a valuation $ v $ is $ \mathcal{A} $ with $ v\rb{x_i} = \overline{b_j} $, then $ v $ does not satisfy $ \psi $, by $ \rb{\ref{eq:8}} $.
\end{enumerate}
\end{proof}
...
\item Suppose $ \mathcal{A} = \ab{A; \overline{E}, \dots} $ is an $ \mathcal{L}^E $-structure and $ \mathcal{A} \vDash \Sigma_E $. Then $ \overline{E} $ is an equivalence relation on $ A $. Denote, for $ a \in A $ $ \widehat{a} = \cb{b \in A \mid \overline{E}\rb{a, b}} $, the equivalence class of $ a $. Let $ \widehat{A} = \cb{\widehat{a} \mid a \in A} $. Make $ \widehat{A} $ into an $ \mathcal{L}^E $-structure $ \widehat{\mathcal{A}} $.
\begin{enumerate}
\item If $ R $ is an $ n $-ary relation symbol and $ \widehat{a_1}, \dots, \widehat{a_n} \in \widehat{A} $ then say $ \overline{R}\rb{\widehat{a_1}, \dots, \widehat{a_n}} $ holds in $ \widehat{\mathcal{A}} $ iff $ \overline{R}\rb{a_1, \dots, a_n} $ holds in $ \mathcal{A} $. This is well-defined by $ \Sigma_E $.
...
\item If $ c $ is a constant symbol, then interpret $ c $ as $ \widehat{\overline{c}} $ in $ \widehat{A} $, where $ \overline{c} $ is the interpretation in $ \mathcal{A} $.
\end{enumerate}
Note that in $ \widehat{\mathcal{A}} $ $ \overline{E}\rb{\widehat{a_1}, \widehat{a_2}} $ iff $ \overline{E}\rb{a_1, a_2} $ in $ \mathcal{A} $, iff $ \widehat{a_1} = \widehat{a_2} $. So $ \widehat{\mathcal{A}} $ is a normal $ \mathcal{L}^E $-structure.
\end{enumerate}
\end{definition}
...
\begin{note}
If $ t $ is any term then $ \widehat{v}\rb{t} = \widehat{v\rb{t}} $ by definition of $ \overline{f} $ on the structure $ \widehat{\mathcal{A}} $.
\end{note}

\begin{proof}
The result \ref{lem:2.6.3} is proved by induction on the number of connectives and quantifiers in $ \phi $.
\begin{enumerate}
\item Base step. $ \phi $ is an atomic formula $ R\rb{t_1, \dots, t_n} $, where $ R $ is an $ n $-ary relation symbol and $ t_1, \dots, t_n $ are terms. Then $ v\sb{\phi} = T $ iff $ \overline{R}\rb{v\rb{t_1}, \dots, v\rb{t_n}} $ holds in $ \mathcal{A} $, iff $ \overline{R}\rb{\widehat{v\rb{t_1}}, \dots, \widehat{v\rb{t_n}}} $ holds in $ \mathcal{A} $ by definition of $ \overline{R} $ in $ \widehat{\mathcal{A}} $, iff $ \overline{R}\rb{\widehat{v}\rb{t_1}, \dots, \widehat{v}\rb{t_n}} $ in $ \mathcal{A} $, iff $ \widehat{v}\sb{\phi} = T $, as required.
\item Inductive step.
\begin{enumerate}
...
Let $ \mathcal{L}^= $ be a first-order language with equality and a binary relation symbol $ \le $.
...
\item the axioms $ \Delta $ for $ \ab{\mathcal{Q}; \le} $,
\end{enumerate}
...
\begin{example}
$ \ab{\Z; \le} $ is not a well ordering. $ \ab{\N; \le} $ is a well ordering.
\end{example}
