...
Given two groups $ G_1 $ and $ G_2 $, we already know one way of combining them, $ G_1 \times G_2 = \cbr{\br{g_1, g_2} \st g_i \in G_i} $.
...
\item $ \ZZ * \ZZ \cong G \cong \FF_2 $.
\item $ \FF_{n + 1} \cong \FF_n * \ZZ $.
\item $ A * B \cong G \cong B * A $.
\end{itemize}
\end{remark}

\begin{lemma}
Let $ A $ and $ B $ be groups. Then there is some $ G $, and $ i_A : A \hookrightarrow G $ and $ i_B : B \hookrightarrow G $ embeddings such that $ G = i_A\br{A} * i_B\br{B} $.
\end{lemma}

\begin{proof}
By taking isomorphic copies if necessary, we may assume that $ A \cap B = \emptyset $. Define the set of elements of $ G $ as all words $ g = g_0 \dots g_n $ with $ g_i \in A \setminus \cbr{e_A} \cup B \setminus \cbr{e_B} $, and $ g_i \in A $ if and only if $ g_{i + 1} \in B $. For $ h = h_0 \dots h_m \in G $ we set inductively
$$ g \cdot h =
\begin{cases}
