...
\begin{definition}
Let $ X $ be a tree. A reduced path is called a \textbf{geodesic}. For $ x, y \in X $, we denote the unique geodesic from $ x $ to $ y $ by $ x - y $. Its length is called the \textbf{distance} between $ x $ and $ y $ and denoted by $ \d\br{x, y} $.
\end{definition}
...
$$ Y
_\sigma = \cbr{x \in X^0 \st \d\br{x, \sigma\br{x}} = \abs{\sigma}}. $$
If $ \abs{\sigma} = 0 $, we write $ Y_\sigma = \mathring{\sigma} $, and if $ \abs{\sigma} > 0 $, we write $ Y_\sigma = \vec{\sigma} $.
\end{definition}
