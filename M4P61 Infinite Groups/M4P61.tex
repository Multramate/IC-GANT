...
\begin{example*}
Let $ X = \CCC_\infty $ and $ 3\ZZ \curvearrowright X $ via $ i \mapsto i + 3 $. Then
$$
\begin{tikzpicture}
\draw (-0.5, 0.5) node[left]{$ 3\ZZ \backslash X \cong $};
\fill (0, 0) circle (0.1) node[left]{$ \sbr{0} $};
\fill (1, 0) circle (0.1) node[right]{$ \sbr{1} $};
\fill (0.5, 1) circle (0.1) node[above]{$ \sbr{2} $};
\draw (0, 0) to (1, 0);
\draw (1, 0) to (0.5, 1);
\draw (0.5, 1) to (0, 0);
\end{tikzpicture}.
...
\label{thm:1.6.19}
A group $ G $ is free if and only if it acts freely on a tree.
\end{theorem}
