...
\end{align*}
so $ \sigma_{k - 1}\br{p^{n + 1}} = \sigma_{k - 1}\br{p}\sigma_{k - 1}\br{p^n} - p^{k - 1}\sigma_{k - 1}\br{p^{n - 1}} $.
\end{itemize}
\end{note*}
...
\begin{definition}
A \textbf{correspondence} on $ X $ is a homomorphism $ \ZZ X \to \ZZ X $. Let $ \Corr X $ be the set of correspondences on $ X $. Equivalently, a correspondence associates to each $ x \in X $, a finite formal sum
$$ \sum_{i = 1}^r a_iy_i, \qquad a_i \in \ZZ, \qquad y_i \in X. $$
\end{definition}

If $ X $ is a finite set $ X = \cbr{x_1, \dots, x_r} $, any correspondence $ T $ can be represented, in a unique way, by the matrix $ M_T $ such that $ T_{x_i} = \sum_{j = 1}^r \br{M_T}_{ij}x_j $, and composition of correspondences is matrix multiplication. Let $ X $ be a set, and let $ \Fun_\CC X $ be the set of functions from $ X $ to $ \CC $. Then $ T \in \Corr X $ acts on $ \Fun_\CC X $ as follows. If $ Tx = \sum_i a_ix_i $ then $ \br{Tf}x = \sum_i a_if\br{x_i} $. Check $ \br{T \circ T'}f = T\br{T'f} $, etc. Let $ \LLL $ be the set of lattices in $ \CC $. The following are correspondences in $ \LLL $.
\begin{itemize}
\item For $ \lambda \in \CC^* $, have
$$ \function[\R_\lambda]{\LLL}{\LLL}{L}{\lambda L}. $$
\item For $ n \in \ZZ_{> 0} $, have
$$ \function[\T_n]{\LLL}{\LLL}{L}{\sum_{L' \subseteq L, \ \text{index} \ n} L'}. $$
...
$$ \correspondence{\text{subgroups of} \ L / \R_nL \ \text{of order} \ n}{\text{subgroups of index} \ n}. $$
\end{itemize}
