...
\begin{theorem}
$ \j $ gives a bijection between $ \HH / \SL_2\br{\ZZ} $ and $ \CC $.
\end{theorem}
...
$$ \ord_\infty g + \dfrac{\ord_\rho g}{3} + \dfrac{\ord_i g}{2} + \sum_{p \in \HH / \SL_2\br{\ZZ}, \ p \nsim \cbr{i, \rho}} \ord_p g = 0. $$
The only possibilities are
\begin{itemize}
...
Know, since $ L $ is even, that $ \Theta_L\br{z + 1} = \Theta_L\br{z} $. It suffices to show
$$ \Theta_L\br{-\dfrac{1}{z}} = z^{\tfrac{n}{2}}\Theta_L\br{z}. $$
Both sides are holomorphic on $ \HH $, so it suffices to show
$$ \Theta_L\br{-\dfrac{1}{it}} = \br{it}^{\tfrac{n}{2}}\Theta_L\br{it}. $$
For $ t \in \RR^\times $, let
$$ L_t = t^{\tfrac{1}{2}} \cdot L, \qquad L_t^\vee = t^{-\tfrac{1}{2}} \cdot L = L_{t^{-1}}, $$
so $ \vol L_t = t^{n / 2} $. By the facts,
$$ \sum_{x \in L_t} e^{-\pi\br{x \cdot x}} = t^{-\tfrac{n}{2}}\sum_{x \in L_{t^{-1}}} e^{-\pi\br{x \cdot x}}, $$
so
$$ \sum_{x \in L} e^{-\pi\br{x \cdot x}t} = t^{-\tfrac{n}{2}}\sum_{x \in L} e^{-\tfrac{\pi\br{x \cdot x}}{2}}. $$
...
Let $ \Theta_L = \sum_{m = 1}^\infty a_mq^m $, where $ a_m $ is the number of ways $ \Q_L $ represents $ 2m $, so $ a_0 = 1 $. Then $ \Theta_L = \E_{n / 2} + g $, where $ g $ is a cusp form. The coefficients of $ \E_{n / 2} $ are constants times $ \sigma_{n / 2 - 1}\br{m} $, which grows like $ m^{n / 2 - 1} $.

\begin{theorem}[Hasse]
If $ g = \sum b_mq^m $ is a cusp form of weight $ k $, then $ \abs{b_m}m^{-k / 2} $ is bounded as $ n \to \infty $.
\end{theorem}
