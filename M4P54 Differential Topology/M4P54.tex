...
Then for all $ \omega_2 \in \Omega_\c^{n - p}\br{M} $,
...
\item The third square. To check $ \br{\mu_U \oplus \mu_V} \circ f = i^* \circ \mu_M $, so
...
Recall $ \dim M = \dim N $, and if $ \omega \in \Omega_\c^n\br{N} $ and if $ \D f_x $ is of rank less than $ n $ for all $ x $, then $ \deg f = 0 $. We are done. In particular we may assume there exists $ x $ such that $ \D f_x $ has rank equal to $ n $. Let $ y = f\br{x} $. By Sard's theorem, we may assume that for all $ x \in f^{-1}\br{y} $, $ \D f_x $ has rank $ n $. By the preimage theorem $ f^{-1}\br{y} $ is a manifold of dimension zero, so
...
$$ \int_{U_i} \eval{f}_{U_i}^*\omega = \sgn\br{\det \D f_x}\int_U \omega, $$
and $ \eval{f}_{U_i}^*\omega $ has support in $ U_i $. Since $ \supp f^*\omega \subset \bigcup_i U_i $,
$$ \int_M f^*\omega = \sum_{i = 1}^k \int_{U_i} f^*\omega = \sum_{i = 1}^k \sgn \br{\det \D f_x}\int_U \omega = \sum_{i = 1}^k \sgn \br{\det \D f_x}\int_M \omega, $$
so $ \deg f = \sum_{i = 1}^k \sgn \br{\det \D f_{x_i}} \in \ZZ $, which does not depend on $ y $, if $ y $ is a regular point.
\end{proof}

As an immediate consequence of the proof of the theorem, we get the following.

\begin{corollary}
Let $ f : M \to N $ be a proper smooth morphism between connected oriented manifolds of dimension $ n $. Assume that $ f $ is not surjective. Then $ \deg f = 0 $.
\end{corollary}
...
$$ \H_f = \br{\dmd{}{2}{x_i}{}{x_j}{}f\br{x}}. $$
Then $ x $ is called \textbf{non-degenerate} if $ \det \H_f\br{x} \ne 0 $. A function such that every critical point is non-degenerate is called a \textbf{Morse function}.
\end{definition}
