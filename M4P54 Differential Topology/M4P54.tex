...
Then $ \omega $ has \textbf{compact support}, if $ \supp \omega $ is compact. For all $ p \ge 0 $,
...
$$ \br{\D\phi_\alpha}_x : \T_xU_\alpha \to \T_{\phi_\alpha\br{x}}\phi_\alpha\br{U} \subset \RR^n $$
is orientation preserving. This is called the positive orientation on the chart $ \br{U_\alpha, \phi_\alpha} $. We define $ \Lambda $ on $ M $, which is a collection of $ \Lambda^+ $ on $ \T_xM $ for all $ x \in M $. Then $ M $ is \textbf{orientable} if there exists an atlas with positive orientation charts. This coincides in assuming that
$$ \det \D\br{\phi_\beta^{-1} \circ \phi_\alpha} > 0, $$
...
& = \int_{\overline{\phi}\br{U \cap \overline{U}}} \br{\phi^{-1}}^* \circ \br{\overline{\phi}^{-1}\overline{\phi}}^*\br{\omega}
...
\item Exercise. \footnote{Exercise}
\item Choose a positively oriented chart $ \br{U_\alpha, \phi_\alpha} $ on $ U_\alpha $, so $ \omega = g_\alpha\phi_\alpha^*\br{\d x_1 \wedge \dots \wedge \d x_n} $ where $ g_\alpha > 0 $. Then $ \int_M \omega = \sum_\alpha \int_{U_\alpha} f_\alpha\omega $ where $ f_\alpha $ is a partition of unity. For all $ x \in M $ there exists $ \alpha $ such that $ x \in U_\alpha $ and $ \int_{U_\alpha} f_\alpha\omega > 0 $, so $ \int_M \omega > 0 $.
