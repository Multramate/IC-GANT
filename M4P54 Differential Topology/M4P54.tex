...
\end{cases}.
$$
By induction on $ n $. If $ n = 1 $, then ok. Assume $ n > 1 $. Let $ U = M \setminus \cbr{N} $ and $ V = M \setminus \cbr{S} $, so $ U \cap V \ne \emptyset $ and $ U \cup V = M $. Then
...
$$
\begin{tikzcd}
& \Coker \d_M^{p - 1} \arrow{r} \arrow{d}{\d_M^p} & \Coker \br{\d_U^{p - 1}, \d_V^{p - 1}} \arrow{r} \arrow{d}{\d_U^p \oplus \d_V^p} & \Coker \d_{U \cap V}^{p - 1} \arrow{r} \arrow{d}{\d_{U \cap V}^p} & 0 \\
0 \arrow{r} & \Ker \d_M^{p + 1} \arrow{r} & \Ker \br{\d_U^{p + 1}, \d_V^{p + 1}} \arrow{r} & \Ker \d_{U \cap V}^{p + 1} &
\end{tikzcd},
$$
which is well-defined, since $ \d^2 = 0 $.
\end{proof}
