...
A \textbf{subcomplex} of a CW-complex $ M $ is a closed subspace of $ M $ which is a union of cells of $ M $. A \textbf{closed cell} is the image of $ \D_n $ in a cell. An \textbf{open cell} is the image of $ \D_n \setminus \partial\D_n $ in a cell. Open cells are not open in $ M $ in general.
...
\begin{definition}
Let $ \br{M, g} $ be a Riemannian manifold, and let $ f : M \to \RR $ be a smooth function. The \textbf{gradient vector field} $ \nabla f $ of $ f $ is the unique vector field on $ M $ such that for all vector fields $ X $, $ g\br{\nabla f, X} = X\br{f} = \D f\br{X} $, the derivative of $ f $ with respect to $ X $.
\end{definition}
