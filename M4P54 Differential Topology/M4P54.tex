...
Let $ M $ and $ N $ be connected oriented manifolds of dimension $ n $, and let $ f : M \to N $ be a proper smooth morphism. Then $ f^* : \H_\c^n\br{N} = \RR \to \H_\c^n\br{M} = \RR $ by Poincar\'e duality and connectedness, so $ f\br{x} = cx $ for some $ \deg f = c \in \RR $. For any $ \omega \in \Omega_\c^n\br{M} $,
$$ \int_M f^*\omega = c\int_M \omega. $$
