...
\item[Inductive step.] The induction hypothesis is $ U \le R^{n - 1} $ implies that $ U \otimes_R M \to R^{n - 1} \otimes_R M $ is injective. Assume $ U \le F = R^n $. Write $ R^n = R \oplus R^{n - 1} $. So we have a short exact sequence
$$ 0 \to R \to R^n \to R^{n - 1} \to 0. $$
We also have a short exact sequence
$$ 0 \to U_1 \to U \to \pi_{R^{n - 1}}\br{U} \to 0, \qquad U_1 = U \cap \br{R \oplus 0^{n - 1}}. $$
Identifying $ R $ with $ R \oplus 0^{n - 1} $, we get a commuting diagram
$$
\begin{tikzcd}[row sep=tiny]
0 \arrow{r} & U_1 \arrow{r} \arrow[subset]{d} & U \arrow{r} \arrow[subset]{d} & \pi_{R^{n - 1}}\br{U} \arrow{r} \arrow[subset]{d} & 0 \\
...
Now $ f $ is injective by Lemma \ref{lem:torsionflat1}, and $ h $ is injective by the inductive hypothesis. The snake lemma tells us that the sequence $ 0 = \Ker f \to \Ker g \to \Ker h = 0 $ is exact at $ \Ker g $, and so $ \Ker g = 0 $. So $ g $ is injective, and this completes the induction.
\end{itemize}
\end{proof}
...
$$
\begin{tikzcd}[row sep=tiny]
0 \arrow{r} & K \arrow{r} \arrow[cong]{d} & F_A \arrow{r} \arrow[subset]{d} & \alpha A \arrow{r} \arrow[subset]{d} & 0 \\
