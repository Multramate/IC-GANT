...
$$ \Omega_{X, \CC}^1 = \Omega_{X, \RR}^1 \otimes_\RR \CC = \Omega_{X, \CC}^{1, 0} \oplus \Omega_{X, \CC}^{0, 1} = \br{\T_X^{1, 0}}^* \oplus \br{\T_X^{0, 1}}^*. $$
...
Thus, for all $ k \ge 0 $
$$ \Omega_{X, \CC}^k = \Lambda^k\Omega_{X, \CC}^1 = \bigoplus_{p + q = k} \Omega_X^{p, q}, \qquad \Omega_X^{p, q} = \Lambda^p\Omega_X^{1, 0} \otimes \Lambda^q\Omega_X^{0, 1}, $$
...
\begin{exercise*}
Assume $ X $ and $ Y $ are biholomorphic complex manifolds. Then $ \H^{p, q}\br{X} = \H^{p, q}\br{Y} $.
\end{exercise*}
...
$$ \nabla\br{f\omega} = f\nabla\br{\omega} + \d f \otimes \omega, \qquad f \in \C^\infty, \qquad \omega \in \C^\infty\br{X, E}. $$
\end{definition}

The following is the idea. Let $ \omega \in \C^\infty\br{X, E} $. Then
$$ \nabla\br{\omega} = \sum_i \eta_i \otimes \omega_i, $$
where $ \eta_i $ are $ 1 $-forms on $ X $ and $ \omega_i $ are sections of $ E $. Let $ x \in X $, and let $ v \in \T_xX $. Then
$$ \nabla_v\br{\omega}_x = \sum_i \eta_i\br{v}\omega_i $$
...
$$ \nabla\br{\sigma} = \sum_{i = 1}^r \nabla\br{f_is_i} = \sum_{i = 1}^r \br{f_i\nabla\br{s_i} + \d f_i \otimes s_i} \in \C^\infty\br{U, \Omega_{X, \CC}^1 \otimes E}. $$
...
Then
$$ \nabla\br{s_j} = \sum_{i = 1}^r a_{ij} \otimes s_i, \qquad a_{ij} \in \C^\infty\br{U, \Omega_{X, \CC}^1}. $$
...
$$ \nabla\br{\sigma} \overset{U}{=} Af + \d f. $$
\begin{itemize}
\item $ A $ depends very much on the choice of the frame.
\item Locally on $ U $, $ \nabla $ is determined by $ A $.
\end{itemize}
Consider another chart $ \br{U', \phi'} $ which also gives a trivialisation of $ E $. So we can choose a corresponding frame $ s_1', \dots, s_r' $. Assume $ \sigma \in \C^\infty\br{U \cap U', E} $. Then $ \sigma \overset{U}{=} \br{f_1, \dots, f_r} $ and
...
$$ \nabla\br{\sigma} \overset{U'}{=} A'f' + \d f'. $$

\lecture{14}{Thursday}{06/02/20}

Let $ g : \br{U \cap U'} \times \CC^r \to \br{U \cap U'} \times \CC^r $ be the transition function from the trivialisation of $ U' $ to the trivialisation of $ U $. Then $ g\br{x} \in \GL_r \CC $ for all $ x \in U \cap U' $, so $ f = gf' $. Denote by $ \D g $ the differential of $ g $. Then
$$ \d f = \d\br{gf'} = \D g \cdot f' + g \d f' = g\br{g^{-1}\D g \cdot f' + \d f'}, $$
by the Leibnitz rule. Thus,
$$ Af + \d f \overset{U}{=} Agf' + g\br{g^{-1}\D g \cdot f' + \d f'} \overset{U}{=} g\br{\br{g^{-1}\D g + g^{-1}Ag}f' + \d f'} \overset{U'}{=} \br{g^{-1}\D g + g^{-1}Ag}f' + \d f', $$
so $ A' = g^{-1}\D g + g^{-1}Ag $. What is $ \nabla^2 $? The idea is
...
Then the \textbf{curvature} operator is
...
$$ \nabla^2\br{\sigma}  = \nabla\br{Af + \d f} = A\br{Af + \d f} + \d\br{A f + \d f} = A \wedge Af + A \d f + \D A \cdot f - A \d f + \d^2f = \br{A \wedge A + \D A}f $$
is $ \C^\infty $-linear, so $ \nabla^2\br{h\sigma} = h\nabla^2\br{\sigma} $. Let
$$ \Theta_\nabla \cdot \sigma = \nabla^2\br{\sigma}, \qquad \Theta_\nabla \overset{U}{=} A \wedge A + DA. $$
...
$$ \abr{\cdot, \cdot}_x : E\br{x} \times E\br{x} \to \CC, \qquad x \in X, $$
such that
$$ \abr{s\br{x}, t\br{x}}_x, \qquad U \subset X, \qquad s, t \in \C^\infty\br{U, E} $$
is a $ \C^\infty $-function with respect to $ x $ on $ U $. The notation is $ h_x = \abr{\cdot, \cdot}_x $. The pair $ \br{E, \abr{\cdot, \cdot}} = \br{E, h} $ is called a \textbf{Hermitian vector bundle}.
\end{definition}

Let $ \br{E, h} $ be a Hermitian vector bundle, and let $ x \in X $. Locally, let $ s_1, \dots, s_n $ be a frame on $ U \ni x $. For any $ x \in U $, $ \abr{s_i\br{x}, s_j\br{x}}_x = h_{ij}\br{x} $ is a smooth function for all $ i $ and $ j $. Then
...
$$ \abr{\sigma\br{x}, \sigma'\br{x}} = f^\intercal H\overline{f'}. $$
Now assume that $ U' $ is a different open set with frame $ \br{s_1', \dots, s_r'} $. Assume $ g : \br{U \cap U'} \times \CC^r \to \br{U \cap U'} \times \CC^r $ is the transition function from the trivialisation on $ U' $ to the trivialisation on $ U $. Let $ H' $ be the matrix of $ h $ with respect to $ s_1', \dots, s_r' $. Then
$$ H' = g^\intercal H\overline{g}. $$
