...
$$ \nabla\br{f\omega} = f \cdot \nabla\br{\omega} + \d f \otimes \omega, \qquad f \in \C^\infty\br{X}, \qquad \omega \in \C^\infty\br{X, E}. $$
\end{definition}
...
be the transition function from the trivialisation of $ U' $ to the trivialisation of $ U $. Then $ g\br{x} \in \GL_r \CC $ for all $ x \in U \cap U' $, so $ f = gf' $. Denote by $ \D g $ the differential of $ g $. Then
...
\begin{notation*}
$$ A \wedge A = \br{\sum_{k = 1}^r \br{a_{ik} \wedge a_{kj}}} $$
is an $ \br{r \times r} $-matrix of $ 2 $-forms.
\end{notation*}
...
is $ \C^\infty $-linear, so $ \nabla^2\br{h\sigma} = h\nabla^2\br{\sigma} $. The \textbf{curvature operator} is
...
$$ \abr{\sigma\br{x}, \sigma'\br{x}}_x = f^\intercal H\overline{f'}. $$
...
$$ \cbr{\sigma, \tau} = \sigma^\intercal H\overline{\tau} = \sum_{i, j = 1}^r h_{ij}\sigma_i \wedge \overline{\tau_j}. $$
...
\begin{definition}
Let $ E $ be a complex Hermitian vector bundle on $ X $, and let $ \nabla $ be a connection on $ X $. We say that $ \nabla $ is \textbf{Hermitian}, or \textbf{compatible with the metric}, if the Leibnitz rule holds, so we have
$$ \d\cbr{\sigma, \tau} = \cbr{\nabla\br{\sigma}, \tau} + \br{-1}^p\cbr{\sigma, \nabla\br{\tau}}, \qquad \sigma \in \C^\infty\br{X, E \otimes \Omega_{X, \CC}^p}, \qquad \tau \in \C^\infty\br{X, E \otimes \Omega_{X, \CC}^q}. $$
\end{definition}

\begin{proposition}
Let $ x \in X $, and let $ s_1, \dots, s_r $ be a local frame of $ E $. Assume $ s_1, \dots, s_r $ is an orthonormal frame around $ x \in X $. Let $ \nabla $ be a connection compatible with the metric, and let $ A $ be the associated matrix with respect to $ s_1, \dots, s_r $. Then
$$ \overline{A}^\intercal = -A, $$
that is $ A $ is \textbf{anti-autodual}.
\end{proposition}

Gram-Schmidt is an algorithm that gives an orthonormal basis of $ E\br{x} $ for all $ x $, which is $ \C^\infty $, say $ s_1', \dots, s_r' $. Then with respect to this frame $ H = \id_r $ because $ \abr{s_i', s_j'}_x = \delta_{ij} $.
...
$$ \cbr{\nabla\br{\sigma}, \tau} = \cbr{A \wedge \sigma + \d\sigma, \tau} = \cbr{A \wedge \sigma, \tau} + \cbr{\d\sigma, \tau} = \br{A \wedge \sigma}^\intercal \wedge \overline{\tau} + \d\sigma^\intercal \wedge \overline{\tau} = \br{-1}^p\sigma^\intercal \wedge A^\intercal \wedge \overline{\tau} + \d\sigma^\intercal \wedge \overline{\tau}, $$
and
$$ \cbr{\sigma, \nabla\br{\tau}} = \sigma^\intercal \wedge \overline{\nabla\br{\tau}} = \sigma^\intercal \wedge \br{\overline{A \wedge \tau + \d\tau}} = \sigma^\intercal \wedge \overline{A} \wedge \overline{\tau} + \sigma^\intercal \wedge \d\overline{\tau}. $$
By the Leibnitz rule, $ \sigma^\intercal \wedge \br{A^\intercal + \overline{A}} \wedge \overline{\tau} = 0 $. This is true for all $ \sigma $ and $ \tau $, so $ A^\intercal + \overline{A} = 0 $.
\end{proof}
...
$$ \D H = A^\intercal H + H\overline{A}, $$
where if $ H = \br{h_{ij}} $ then $ \D H = \br{\d h_{ij}} $. A hint is to do the same calculation.
\end{exercise*}

\begin{theorem}
If $ E \to X $ is a complex Hermitian veector bundle, then there exists a connection $ \nabla $ compatible with $ h $.
\end{theorem}

\subsection{Holomorphic vector bundles}

Let $ X $ be a complex manifold, and let $ E $ be holomorphic vector bundle. Then $ \Omega_{X, \CC}^1 \xrightarrow{\sim} \Omega_X^{1, 0} \oplus \Omega_X^{0, 1} $. Given $ \nabla : \C^\infty\br{X, E} \to \C^\infty\br{X, \Omega_{X, \CC}^1 \otimes E} $, we can define
