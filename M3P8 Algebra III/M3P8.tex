...
\item Given a submodule $ N' $ of $ M / N $, let $ \tilde{N'} $ be its preimage in $ N $ under the canonical quotient map $ f : M \to M / N $. We have a surjection from $ \tilde{N'} $ to $ N' $ induced by $ f $. $ \tilde{N'} \subseteq M $, so $ \tilde{N'} $ is finitely generated, say by $ n_1, \dots, n_s $. Claim that $ f\rb{n_1}, \dots, f\rb{n_s} $ generate $ N' $. Given $ n \in N' $, there exists $ \tilde{n} \in \tilde{N'} $ such that $ f\rb{\tilde{n}} = n $. Write $ \tilde{n} = r_1n_1 + \dots + r_sn_s $ for $ r_i \in R $. Then $ n = f\rb{\tilde{n}} = r_1f\rb{n_1} + \dots + r_sf\rb{n_s} $.
\end{enumerate}
\end{proof}
...
\begin{proof}
Let $ M' $ be a submodule of $ M $. Then $ M' \cap N $ is a submodule of $ M $, hence finitely generated. Let $ a_1, \dots, a_s \in M' \cap N $ generate $ M' \cap N $. Let $ \overline{M'} $ denote the image of $ M' $ in $ M / N $. This is a submodule of $ M / N $ and thus finitely generated. Let $ \overline{b_1}, \dots, \overline{b_t} \in \overline{M'} \subseteq M / N $ generate $ \overline{M'} $, and choose elements $ b_1, \dots, b_t $ of $ M' $ mapping to $ \overline{b_1}, \dots, \overline{b_t} $ in $ M / N $, respectively. We now show that $ a_1, \dots, a_s, b_1, \dots, b_t $ is a generating set for $ M' $, proving the claim. Given any $ m \in M' $, let $ \overline{m} $ be its image in $ M / N $ under $ f : M' \to \overline{M'} $. Then we can write $ \overline{m} $ as a sum $ r_1\overline{b_1} + \dots + r_t\overline{b_t} $ for some $ r_1, \dots, r_t \in R $. Let $ m' = m - r_1b_1 - \dots - r_tb_t $. Then the image of $ m' $ in $ M / N $ is $ f\rb{m'} = \overline{m} - r_1\overline{b_1} - \dots - r_t\overline{b_t} = 0 $. So $ m' $ lies in $ N $. $ m \in M', r_1b_1 \in M', \dots, r_tb_t \in M' $ so $ m' $ also lies in $ M' $. So it lies in $ M' \cap N $. We can thus write $ m' $ as $ q_1a_1 + \dots + q_sa_s $ for some $ q_1, \dots, q_s \in R $. We then have
...
iff leading coefficient of $ P\rb{X} $ is in the ideal generated by $ a_1, \dots, a_r $. 
...
An observation is that if $ R $ is Noetherian and $ I \subseteq R $ is an ideal, then $ R / I $ is Noetherian. Let $ J $ be an ideal of $ R / I $ and $ \tilde{J} $ be preimage of $ J $ in $ R $. There exist $ \tilde{j_1}, \dots, \tilde{j_n} $ generating $ \tilde{J} $ over $ R $. Let $ j_i = \tilde{j_i} + I \in R / I $. These lie in $ J $ and generate $ J $ over $ R / I $. In particular, any quotient of polynomial ring over a field or PID is Noetherian. Indeed, since any quotient of a Noetherian ring is Noetherian, we can say more.
