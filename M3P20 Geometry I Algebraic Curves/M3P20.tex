...
$$ \partial_\lambda\rb{\lambda^d \cdot P\rb{x_0, x_1, x_2}} = d\lambda^{d - 1}P\rb{x_0, x_1, x_2}, \qquad \partial_\lambda\rb{P\rb{\lambda x_0, \lambda x_1, \lambda x_2}} = \sum_{i = 0}^2 x_i\partial_{x_i}P\rb{\lambda x_0, \lambda x_1, \lambda x_2}. $$
By the equality above we have
$$ \sum_{i = 0}^2 x_i\partial_{x_i}P\rb{\lambda x_0, \lambda x_1, \lambda x_2} = d\lambda^{d - 1}P\rb{x_0, x_1, x_2}, $$
...
\begin{exercise}
Let $ C \subseteq \P^2 $ be a projective curve and let $ \Psi : \P^2 \to \P^2 $ be a projective transformation. Show that
\begin{enumerate}
\item $ C $ is smooth if and only if $ \Psi\rb{C} $ is smooth, and
\item if $ p \in C $ is a smooth point and $ l $ is the tangent line of $ C $ at $ p $ then $ \Psi\rb{C} $ is smooth at the point $ \Psi\rb{p} $ and $ \Psi\rb{l} $ is the tangent line of $ \Psi\rb{C} $ at $ \Psi\rb{p} $.
\end{enumerate}
\end{exercise}

\begin{remark}
As this last exercise suggests, one should think of curves only differing from each other via a projective transformation to be the same, since projective transformations preserve their properties. These transformations should be thought of as a change of projective coordinates.
\end{remark}
