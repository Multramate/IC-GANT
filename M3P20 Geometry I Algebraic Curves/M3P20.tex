...
The previous two lemmas can be used to define a topology on $ \P^n $. Let $ U \subseteq \P^n $, then $ U $ is open if and only if $ \phi_i^{-1}\rb{U \cap U_i} \subseteq \C^n $ is open in $ \C^n $ for any $ i = 0, \dots, n $.
...
\begin{exercise}
We can define another topology on $ \P^n $, as follows. A subset $ U \subseteq \P^n $ is open if and only if its preimage $ \pi^{-1}\rb{U} $ is open in $ W $, where $ \pi : W \to \P^n $ is the map defined by $ \rb{x_0, \dots, x_n} \mapsto \sb{x_0, \dots, x_n} $. Show that this indeed defines a topology, and that this topology coincides with the one defined above using the maps $ \phi_i $.
\end{exercise}
...
$$ \P^n = U_i \sqcup P_i \cong \C^n \cup \P^{n - 1} = \dots = \C^n \cup \dots \cup \C^0. $$
$ P_i $ is called the hyperplane at infinity.

\begin{example}
In $ \R $, $ \P^1\rb{\R} = \R \cup \R $ by the identification $ r \mapsto 1 / r $, so $ \P^1\rb{\R} \cong S^1 $, while $ \P^2\rb{\R} = \R^2 \cup \P^1\rb{\R} $ has a circle at infinity identified by directions.
\end{example}
