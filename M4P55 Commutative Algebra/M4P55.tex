...
\item Pick up any $ x \in C $. Then
$$ x^n + b_{n - 1}x^{n - 1} + \dots + b_0 = 0, \qquad b_i \in B. $$
Since $ b_0, \dots, b_{n - 1} $ are integral over $ A $, by $ 2 $ we get that $ A\sbr{b_0, \dots, b_{n - 1}} $ is a finitely generated $ A $-module. Then $ A\sbr{b_0, \dots, b_{n - 1}, x} $ is also finitely generated as a module over $ A\sbr{b_0, \dots, b_{n - 1}} $. By $ 1 $, $ A\sbr{b_0, \dots, b_{n - 1}, x} $ is a finitely generated as an $ A $-module. Now Theorem \ref{thm:14.1}.$ 3 $ says that $ x $ is integral over $ A $.
...
\hfill
\begin{itemize}
\item If $ x = \sqrt{a} $, $ y = \sqrt{b} $, and $ z = \sqrt{c} $ for $ a, b \in \QQ $, then $ \sqrt{a} + \sqrt{b} + \sqrt{c} $ is algebraic over $ \QQ $.
\item $ \sqrt[3]{2} + \sqrt[5]{7} $ is algebraic over $ \QQ $.
\end{itemize}
These follow immediately from Lemma \ref{lem:14.3}.$ 4 $.
\end{example*}
